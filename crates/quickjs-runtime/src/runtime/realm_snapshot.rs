...
impl RealmSnapshot {
    ...
    pub(super) fn capture(runtime: &Runtime, realm: RealmId) -> Self {
        ...
        for (object, identity) in [
            ...
            (
                temporal.plain_time_prototype,
                "%Temporal.PlainTime.prototype%",
            ),
            (
                temporal.plain_month_day_prototype,
                "%Temporal.PlainMonthDay.prototype%",
            ),
            (
                temporal.plain_year_month_prototype,
                "%Temporal.PlainYearMonth.prototype%",
            ),
            (
                temporal.zoned_date_time_prototype,
            ...
            (iterators.helper_prototype, "%IteratorHelperPrototype%"),
            (
...
mod tests {
    ...
    const REALM_NODES: usize = 883;
    const REALM_PROPERTIES: u64 = 2_670;
    const REALM_SNAPSHOT_FINGERPRINT: u64 = 13_361_100_037_210_931_474;
    ...
    fn realm_snapshots_normalize_local_identity_and_preserve_isolation() {
        ...
        assert_eq!(
            first_atoms,
            AtomUsage {
                live_atoms: PREDEFINED_ATOM_COUNT + 393,
                live_description_code_units: PREDEFINED_DESCRIPTION_CODE_UNITS + 3_543,
                interner_slots: PREDEFINED_INTERNER_SLOTS + 393,
            }
        );
