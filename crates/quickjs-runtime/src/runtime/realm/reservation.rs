...
mod tests {
    ...
    fn reservation_plan_matches_the_characterized_realm_delta() {
        ...
        assert_eq!(plan.dynamic_atoms, 253);
        assert_eq!(plan.dynamic_atom_code_units, 2_167);
        assert_eq!(plan.realms, 1);
        assert_eq!(plan.objects, 44);
        assert_eq!(plan.functions, 406);
        assert_eq!(plan.global_bindings, 0);
        assert_eq!(plan.object_properties, 1_366);
        assert_eq!(plan.journal_entries, 704);
    }
}
