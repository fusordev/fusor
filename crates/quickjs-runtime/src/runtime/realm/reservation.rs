...
mod tests {
    ...
    fn reservation_plan_matches_the_characterized_realm_delta() {
        ...
        assert_eq!(plan.dynamic_atoms, 242);
        assert_eq!(plan.dynamic_atom_code_units, 2_012);
        assert_eq!(plan.realms, 1);
        assert_eq!(plan.objects, 42);
        assert_eq!(plan.functions, 389);
        assert_eq!(plan.global_bindings, 0);
        assert_eq!(plan.object_properties, 1_311);
        assert_eq!(plan.journal_entries, 674);
    }
}
