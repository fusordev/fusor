...
mod tests {
    ...
    fn reservation_plan_matches_the_characterized_realm_delta() {
        ...
        assert_eq!(plan.dynamic_atoms, 393);
        assert_eq!(plan.dynamic_atom_code_units, 3_516);
        assert_eq!(plan.realms, 1);
        assert_eq!(plan.objects, 78);
        assert_eq!(plan.functions, 784);
        assert_eq!(plan.global_bindings, 0);
        assert_eq!(plan.object_properties, 2_612);
        assert_eq!(plan.journal_entries, 1_256);
    }
}
