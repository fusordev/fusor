...
mod tests {
    ...
    fn reservation_plan_matches_the_characterized_realm_delta() {
        ...
        assert_eq!(plan.dynamic_atoms, 330);
        assert_eq!(plan.dynamic_atom_code_units, 2_802);
        assert_eq!(plan.realms, 1);
        assert_eq!(plan.objects, 65);
        assert_eq!(plan.functions, 612);
        assert_eq!(plan.global_bindings, 0);
        assert_eq!(plan.object_properties, 2_067);
        assert_eq!(plan.journal_entries, 1_008);
    }
}
