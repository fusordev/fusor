...
fn atom_failure_rolls_back_the_complete_installation() {
    ...
    let atom_limits = AtomLimits::new(
        PREDEFINED_ATOM_COUNT + 194,
        PREDEFINED_DESCRIPTION_CODE_UNITS + 1_540,
        PREDEFINED_INTERNER_SLOTS + 194,
    );
    let mut runtime =
...
fn public_root_metadata_preflight_is_failure_atomic() {
    ...
    for (limits, resource, limit, observed) in [
        (
            RuntimeLimits::default().with_max_heap_objects(35),
            RuntimeResource::HeapObjects,
            35,
            36,
        ),
        (
            RuntimeLimits::default().with_max_object_properties(1_027),
