...
fn symbol_creation_returns_the_atom_limit_error_without_partial_usage() {
    let limits = AtomLimits::new(
        PREDEFINED_ATOM_COUNT + 81,
        PREDEFINED_DESCRIPTION_CODE_UNITS + 701,
        PREDEFINED_INTERNER_SLOTS + 81,
    );
    let mut runtime =
    ...
    assert_eq!(
        ...
        AtomError::LiveAtomLimit {
            current: PREDEFINED_ATOM_COUNT + 81,
            additional: 1,
            maximum: PREDEFINED_ATOM_COUNT + 81,
        }
    );
