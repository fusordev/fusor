...
fn safe_points_reclaim_transient_acyclic_closures_before_heap_limits() {
    ...
    let mut runtime =
        Runtime::try_new(RuntimeLimits::default().with_max_heap_functions(163)).expect("runtime");
    let realm = runtime.create_realm().expect("realm");
    let mut context = runtime.context(&realm).expect("context");
    let outer = context.instantiate(authority).expect("outer");
    ...
    for _ in 0..8 {
        ...
        assert_eq!(context.runtime_usage().heap_functions(), 163);
    }
}
...
fn captured_cell_writes_dirty_the_safe_point_collector() {
    ...
    let mut runtime =
        Runtime::try_new(RuntimeLimits::default().with_max_heap_functions(164)).expect("runtime");
    let realm = runtime.create_realm().expect("realm");
    let mut context = runtime.context(&realm).expect("context");
    ...
    assert_eq!(context.runtime_usage().heap_functions(), 164);
    drop(replacement);
}
