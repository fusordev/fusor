...
use fusor_bytecode::{CompilerInitializationPolicy, ModuleBindingOrigin, ModuleImportName};
    ...
    for (index, binding) in decl.bindings().iter().enumerate() {
        ...
        let function = runtime
            .create_module_closure(
                ...
                code_id,
                &authority,
                fusor_bytecode::FunctionTemplateId::new(initializer),
                &cells,
                &parent_environment,
            )
...
fn resolve_export(
    ...
    for entry in syntax.export_entries() {
        let export_matches = match entry.export_name() {
            fusor_frontend::ModuleExportName::Name(name) => {
                name_units_eq_utf8(name.code_units(), export_name)
            }
            fusor_frontend::ModuleExportName::Default(_) => export_name == b"default",
            _ => false,
        };
        if !export_matches {
            continue;
        ...
        match entry.role() {
            fusor_frontend::ModuleExportEntryRole::Local => {
                let local_bytes = match entry.local_name() {
                    fusor_frontend::ModuleExportLocalName::Name(name) => {
                        units_to_utf8(name.code_units())
                    }
                    fusor_frontend::ModuleExportLocalName::SyntheticDefault => {
                        SYNTHETIC_DEFAULT_BINDING_NAME.to_vec()
                    }
                    fusor_frontend::ModuleExportLocalName::Null => {
                        resolve_set.pop();
                        return Ok(ExportResolution::Null);
                    }
                    _ => {
            ...
            fusor_frontend::ModuleExportEntryRole::Indirect => {
                ...
                if matches!(
                    entry.import_name(),
                    fusor_frontend::ModuleExportImportName::All
                ) {
                    resolve_set.pop();
                ...
                let import_bytes = match entry.import_name() {
                    fusor_frontend::ModuleExportImportName::Name(name) => {
                        units_to_utf8(name.code_units())
                    }
                    fusor_frontend::ModuleExportImportName::Default(_) => b"default".to_vec(),
                    _ => {
                        resolve_set.pop();
                        return Ok(ExportResolution::Null);
                    }
            ...
            fusor_frontend::ModuleExportEntryRole::Star => continue,
            _ => continue,
        }
    }
    ...
    if export_name != b"default" {
        ...
        for entry in syntax.export_entries() {
            if entry.role() != fusor_frontend::ModuleExportEntryRole::Star {
                continue;
            }
...
fn module_export_names_inner(
    ...
    for entry in syntax.export_entries() {
        let export_name = match entry.export_name() {
            fusor_frontend::ModuleExportName::Name(name) => units_to_utf8(name.code_units()),
            fusor_frontend::ModuleExportName::Default(_) => b"default".to_vec(),
            fusor_frontend::ModuleExportName::Null => continue,
            _ => continue,
        };
        if entry.role() == fusor_frontend::ModuleExportEntryRole::Star {
            continue;
...
fn resolve_module_imports(
    ...
    let syntax = runtime
        ...
        .clone();
    for entry in syntax.export_entries() {
        if entry.role() != fusor_frontend::ModuleExportEntryRole::Indirect {
            continue;
        }
        let export_name = match entry.export_name() {
            fusor_frontend::ModuleExportName::Name(name) => units_to_utf8(name.code_units()),
...
fn initial_cell_value(binding: &fusor_bytecode::ModuleBindingDescriptor) -> SlotValue {
    ...
    let export_name = if import.is_default() {
        ...
        record
            ...
            .map_or(local_name.clone(), |entry| match entry.import_name() {
                fusor_frontend::ModuleImportName::Name(name) => units_to_utf8(name.code_units()),
                fusor_frontend::ModuleImportName::Default(_) => b"default".to_vec(),
                _ => local_name.clone(),
            })
    } else {
