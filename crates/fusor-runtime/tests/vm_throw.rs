...
use fusor_bytecode::{FunctionTemplateId, VerificationLimits};
...
fn compile(
    ...
    root_name: &str,
    source_name: &str,
) -> Arc<fusor_bytecode::VerifiedBytecode> {
    with_parsed_program(
        source,
...
fn escaping_exception(result: Result<fusor_runtime::JsValue, ExecutionError>) -> JsException {
    match result {
