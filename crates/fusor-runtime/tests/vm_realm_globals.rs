...
fn realm_global_function_authority(
    ...
    let root_flow = Arc::new(
        verify_compiler_control_flow(
            ...
            .with_constant_layout(CompilerConstantLayout::new(Arc::from([
                fusor_bytecode::CompilerConstantKind::Function,
            ]))),
            VerificationLimits::default(),
        )
            ...
            UnverifiedCompilerFunctionGraph::new(
                ...
                Arc::from([
                    UnverifiedCompilerFunction::new(
                        Arc::clone(&root_flow),
                        Arc::from([fusor_bytecode::CompilerConstant::Function(
    ...
    let mapped_source = |flow: &fusor_bytecode::VerifiedControlFlow,
...
fn runtime() -> (Runtime, fusor_runtime::Realm) {
    let mut runtime = Runtime::try_new(RuntimeLimits::default()).expect("runtime");
    let realm = runtime.create_realm().expect("realm");
    (runtime, realm)
}

fn assert_number(value: &fusor_runtime::JsValue, expected: f64) {
    assert_eq!(
...
fn global_var_property_limit_failure_is_atomic() {
    ...
    assert!(matches!(
        error,
        DynamicFunctionScriptError::Install(fusor_runtime::InstallError::LimitExceeded {
