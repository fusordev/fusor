...
use fusor_bytecode::{FinalOpcode, FunctionTemplateId};
...
fn orphaned_and_wrong_kind_handles_fail_without_runtime_access() {
    let value = {
        ...
        assert!(matches!(
            ...
            Err(fusor_runtime::HandleError::WrongValueKind {
    ...
    assert!(matches!(
        value.kind(),
        Err(fusor_runtime::HandleError::Orphaned {
...
fn failed_multi_capture_closure_creation_is_failure_atomic() {
    ...
    assert!(matches!(
        ...
        ExecutionError::LimitExceeded {
            resource: fusor_runtime::RuntimeResource::BindingCells,
            limit: 1,
            observed: 2,
        }
    ));
