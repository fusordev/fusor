...
use fusor_runtime::{JsNumber, ModuleKey, Runtime, RuntimeLimits};
...
fn facade_module_evaluation_error_reports_the_async_outcome() {
    ...
    assert!(
        fusor::module_evaluation_error(&context, "fulfill.mjs").is_none(),
    ...
    pump_dynamic_imports(&mut context, &mut loader, ScriptLimits::default())
        .expect("pump drains the rejection continuation");
    let error = fusor::module_evaluation_error(&context, "reject.mjs")
