...
use fusor::{ScriptEvaluationError, ScriptLimits, evaluate_script};
use fusor_runtime::{
...
fn compile_script_accepts_scripts_without_executing_them() {
    let result = fusor::compile_script(
...
fn compile_script_rejects_syntax_errors() {
    let error = fusor::compile_script("function (", "broken.js", ScriptLimits::default())
        .expect_err("syntax errors must be rejected");
    let fusor::ScriptCompileError::Frontend(frontend) = &error else {
