...
use quickjs_diagnostics::{SourceMap, SourceMapErrorKind, SourceMapPosition, SourceRegistry};
...
fn url_only_index_sections_are_a_non_panicking_unresolved_boundary() {
    ...
    assert_eq!(error.code(), "quickjs::sourcemap::unresolved_section");
}
...
fn malformed_maps_return_structured_errors_without_unwinding() {
    ...
    assert_eq!(
        version_error.code(),
        "quickjs::sourcemap::unsupported_version"
    );
