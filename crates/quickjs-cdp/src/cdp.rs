...
use quickjs_runtime::{Context, DebugExecutionSnapshot, DebuggerHook};
use serde_json::{Value, json};
use sha1::{Digest, Sha1};
use tokio::sync::mpsc as tokio_mpsc;

const TARGET_ID: &str = "quickjs";
const WEBSOCKET_PATH: &str = "/devtools/page/quickjs";
        ...
        match method {
            "Runtime.enable" => {
                self.emit(json!({
                    ...
                    "params": {"context": {"id": 1, "origin": "", "name": "qjs", "auxData": {"isDefault": true}}}
                }));
...
fn version_metadata(port: u16) -> Value {
    json!({
        ...
        "User-Agent": "qjs Chrome DevTools Protocol",
...
pub(crate) fn script_compile_error_position(
    ...
    match error {
        quickjs::ScriptCompileError::Frontend(frontend) => {
...
mod tests {
    ...
    fn parses_a_websocket_upgrade_request() {
        let request = b"GET /devtools/page/quickjs HTTP/1.1\r\nHost: localhost\r\nUpgrade: websocket\r\nSec-WebSocket-Key: abc\r\n\r\n";
