...
fn every_executable_opcode_round_trips_and_rejects_truncation_at_every_byte() {
    for &opcode in ALL_FINAL_OPCODES {
        ...
        let decoded = quickjs_bytecode::decode_instruction(bytes, BytecodePc::ZERO)
        ...
        for cut in 0..bytes.len() {
            let error = quickjs_bytecode::decode_instruction(&bytes[..cut], BytecodePc::ZERO)
...
fn decoder_rejects_reserved_unknown_and_out_of_bounds_program_counters() {
    assert_eq!(
        quickjs_bytecode::decode_instruction(&[0], BytecodePc::ZERO),
    ...
    assert_eq!(
        quickjs_bytecode::decode_instruction(&[244], BytecodePc::ZERO),
        Err(DecodeError::InvalidOpcode {
            pc: BytecodePc::ZERO,
            opcode_byte: 244,
    ...
    assert_eq!(
        quickjs_bytecode::decode_instruction(
...
fn decoder_iterator_tracks_typed_pcs_and_stops_after_an_error() {
    ...
    let malformed_bytes = [244, FinalOpcode::Nop.encoded_byte()];
    ...
    for leading_byte in u8::MIN..=u8::MAX {
        for byte_len in 0..=MAX_INSTRUCTION_BYTES {
            ...
            for pc in 0..=u32::try_from(byte_len + 1).expect("bounded test length") {
                let _ = quickjs_bytecode::decode_instruction(bytes, BytecodePc::new(pc));
            }
            let _ = quickjs_bytecode::decode_instruction(bytes, BytecodePc::new(u32::MAX));
