...
) -> Result<fusor_bytecode::VerifiedControlFlow, VerificationError> {
    verify_control_flow(
...
fn variable_reference_count_obeys_the_quickjs_structural_maximum() {
    ...
    assert_eq!(
        ...
        &VerificationErrorKind::MetadataCountOutOfRange {
            domain: fusor_bytecode::FunctionCountDomain::VariableReferences,
            value: 65_535,
            maximum: 65_534,
        }
    );
...
fn complete_static_validation_precedes_function_kind_rejection() {
    ...
    assert_eq!(
        ...
        &VerificationErrorKind::IndexOutOfBounds {
            domain: fusor_bytecode::OperandIndexDomain::Local,
            index: 0,
            len: 0,
        }
    );
