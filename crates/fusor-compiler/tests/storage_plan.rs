...
fn script(source: &str) -> fusor_compiler::StoragePlan {
    script_with_goal(source, GlobalScriptGoal::new())
}

fn script_with_goal(source: &str, goal: GlobalScriptGoal) -> fusor_compiler::StoragePlan {
    with_parsed_program(
        source,
...
fn module(source: &str) -> fusor_compiler::StoragePlan {
    with_parsed_program(
        source,
...
fn indirect_eval(source: &str) -> fusor_compiler::StoragePlan {
    with_parsed_program(
        source,
...
fn plan_escapes_the_frontend_arena_with_dense_executable_ids() {
    ...
    assert_eq!(
        ...
        Some(fusor_frontend::Span::new(25, 30))
    );
...
fn ordinary_namespace_and_default_module_bindings_have_no_oxc_identity_surface() {
    ...
    assert_send_sync::<fusor_compiler::StoragePlan>();
...
fn parameter_arguments_resolves_before_body_lexical_arguments() {
    ...
    let targets = plan
        ...
        .unwrap()
        .iter()
        .map(fusor_compiler::ResolvedReference::binding)
        .collect::<Vec<_>>();
...
fn expression_free_destructured_parameters_use_raw_positions_and_local_bindings() {
    ...
    assert!(
        bindings
            .iter()
            .any(fusor_compiler::BindingStorage::is_arguments_object)
    );
}
