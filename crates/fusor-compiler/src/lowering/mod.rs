...
impl<'arena> CompilationContext<'_, 'arena, '_> {
    ...
    fn build_module_declaration_record(
        ...
        tree_layout: &FunctionTreeLayout,
    ) -> Result<Option<fusor_bytecode::UnverifiedModuleDeclarationRecord>, LeafCompilationError>
    {
        ...
        for binding in root_function.module_bindings.iter() {
            let mut descriptor = fusor_bytecode::UnverifiedModuleBindingDescriptor::new(
                binding.atom,
        ...
        for (index, request) in self.unit.module_syntax().requests().iter().enumerate() {
            ...
            requests.push(fusor_bytecode::ModuleRequestDescriptor::new(
                specifier,
                request.attributes().is_some(),
            ));
        }
        Ok(Some(
            fusor_bytecode::UnverifiedModuleDeclarationRecord::new(
                bindings.into(),
                requests.into(),
            ),
        ))
