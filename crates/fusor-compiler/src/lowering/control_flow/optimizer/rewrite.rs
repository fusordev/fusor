...
struct RebuiltInstructions {
    bytecode: Vec<u8>,
    instruction_pcs: Vec<fusor_bytecode::BytecodePc>,
    origins: Vec<usize>,
...
fn allocate_labels(
    ...
    sources: &[SourceInstruction],
    facts: &ControlFlowFacts,
) -> Result<(Vec<Option<AssemblerLabel>>, Vec<fusor_frontend::Span>), LeafCompilationError> {
...
fn finish_assembler(
    ...
    origins: &[usize],
    sources: &[SourceInstruction],
    label_spans: &[fusor_frontend::Span],
) -> Result<fusor_bytecode::AssembledBytecode, LeafCompilationError> {
    match plan.finish() {
...
fn emitted_span(
    ...
    origins: &[usize],
    sources: &[SourceInstruction],
) -> Result<fusor_frontend::Span, LeafCompilationError> {
