...
impl CompilationContext<'_, '_, '_> {
    ...
    pub(in crate::lowering) fn compiled_closure_definitions(
        ...
        for global in realm_globals {
            ...
            let mut definition = match global.binding {
                fusor_bytecode::CompilerClosureBinding::Captured(policy) => {
                    VerifiedClosureVariableDefinition::new(Some(name), policy, source)
                }
                fusor_bytecode::CompilerClosureBinding::RealmGlobal(policy) => {
        ...
        for binding in module_bindings {
            let source = match binding.source {
                ...
                CompiledModuleBindingSource::ParentClosure(slot) => {
                    fusor_bytecode::CompilerClosureSource::ParentClosure(u32::from(slot))
                }
            };
            let mut definition =
    ...
    fn module_import_descriptor(
        ...
        atom: fusor_bytecode::AtomPoolIndex,
    ) -> Result<Option<fusor_bytecode::ModuleImportName>, LeafCompilationError> {
        if !matches!(
            descriptor.origin,
        ...
        let import = match entry.import_name() {
            fusor_frontend::ModuleImportName::Namespace => {
                fusor_bytecode::ModuleImportName::namespace(request)
            }
            fusor_frontend::ModuleImportName::DeferredNamespace => {
                fusor_bytecode::ModuleImportName::deferred_namespace(request)
            }
            fusor_frontend::ModuleImportName::Default(_) => {
                fusor_bytecode::ModuleImportName::default(request)
            }
            fusor_frontend::ModuleImportName::Name(_) => {
                ...
                fusor_bytecode::ModuleImportName::named(request, atom)
            }
            _ => {
                return Err(LeafCompilationError::SemanticInvariant {
