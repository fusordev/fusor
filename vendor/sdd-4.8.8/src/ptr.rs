...
impl<'g, T> Ptr<'g, T> {
    ...
    pub fn as_ref(&self) -> Option<&'g T> {
        let ptr = Tag::unset_tag(self.ptr);
        if ptr.is_null() {
            return None;
        }
        unsafe { Some(&*ptr) }
    ...
    pub const unsafe fn as_ref_unchecked(&self) -> Option<&'g T> {
        unsafe { RefCounted::inst_ptr(self.ptr).as_ref() }
    }
    ...
    pub fn as_ptr(&self) -> *const T {
        RefCounted::inst_ptr(Tag::unset_tag(self.ptr))
    }

    /// Creates a raw pointer to the underlying instance without checking tag bits.
    ///
    /// # Safety
    ///
    /// This [`Ptr`] must not have any tag bits set, otherwise dereferencing the pointer may lead to
    ...
    pub fn get_shared(self) -> Option<Shared<T>> {
        unsafe {
            if let Some(ptr) = NonNull::new(Tag::unset_tag(self.ptr).cast_mut()) {
    ...
    /// Creates a new [`Ptr`] from a [`RefCounted`] pointer.
    #[inline]
