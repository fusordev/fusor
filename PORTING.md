...
- [~] Temporal: `%Temporal.Instant%`, `%Temporal.Duration%`, `Date.prototype.toTemporalInstant`, and branded `%Temporal.PlainDate%`/`%Temporal.PlainDateTime%` cores share `temporal_rs = 0.2.5`. PlainDate supports ordered conversion, string-only calendar validation, new-target allocation, ISO/string/branded/property-bag `from`/`compare`, calendar-derived accessors, optioned ISO string/JSON/non-Intl locale output, `equals`, date-duration `add`/`subtract`, partial-field `with`, `toPlainDateTime`, `withCalendar`, and `until`/`since`. PlainDateTime has its constructor, time defaults, core calendar/date/time accessors, optioned ISO string/JSON/non-Intl locale output, primitive rejection, ISO/string/branded/PlainDate/property-bag `from`/`compare`, `equals`, duration `add`/`subtract`, partial-field `with`, `round`, `toPlainDate`/`toPlainTime`, `withCalendar`, and `until`/`since`. PlainTime has branded slots, constructor/new-target behavior, accessors, optioned output, ISO/string/branded/PlainDateTime/property-bag conversion, arithmetic, `with`, `round`, and rounded `until`/`since`; PlainDate bridges its PlainTime slots directly. PlainMonthDay and PlainYearMonth have constructors, accessors, ordered ISO/string/branded/property-bag `from`/`compare`/`equals`, partial-field `with`, `toPlainDate`, optioned string/JSON/non-Intl locale output; YearMonth additionally has duration `add`/`subtract` and `until`/`since`. Property bags preserve observable field conversion order; `from`/arithmetic defer overflow validation, `with` prepares fields before options, rounding reads/coerces all options resumably, differences convert the operand before all four difference settings, and kernel Duration outputs normalize to ECMAScript Number fields. Focused upstream PlainMonthDay is 398/398 and PlainYearMonth is 1018/1018: their calendar-object paths read the internal calendar of every admitted branded Temporal value, including `%Temporal.ZonedDateTime%`’s initial constructor/string-branded `from`, complete branded-slot getters (calendar, time zone, epoch, offset, date/time, and calendar fields), branded `toInstant`/`toPlainDate`/`toPlainTime`/`toPlainDateTime`/`startOfDay` projections, branded/string `equals`/static `compare`, `toJSON`, non-Intl `toLocaleString`, primitive rejection, `toString`, and `withTimeZone`. ZonedDateTime `toString` reads/coerces `calendarName`, `fractionalSecondDigits`, `offset`, `roundingMode`, `smallestUnit`, and `timeZoneName` in order before validation and renders through the shared kernel; its focused upstream `toString` cohort is 124/124, and string-only `withTimeZone` is 32/32. Focused ZonedDateTime is 768/1796 (42.76%); generic property bags/options/methods, Calendar, and remaining Temporal types are next.
