...
- [~] `Iterator`, `%Iterator.prototype%`, `Iterator.from`, `toArray`, and shared `%IteratorHelperPrototype%` state for `map`/`filter`/`take`/`drop`/`flatMap` implement retained direct records, lazy resumable coercion/callbacks, indexes, reentrancy rejection, GC tracing, and required close/error ordering. `flatMap` persists one active inner record across yields and closes inner before outer on abrupt return. Pinned Test262 is 38/38 `from`, 36/36 `toArray`, 72/72 `map`, 74/74 `filter`, 60/60 admitted `take`, 62/62 admitted `drop`, and 88/88 `flatMap`; six modes per limit helper are policy-excluded because they retain the removed `MAX_SAFE_INTEGER` bound. Next add consuming helpers; keep async helpers separate until job-queue interaction has focused coverage.
