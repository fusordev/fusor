...
- [~] Temporal: `%Temporal.Instant%`, `%Temporal.Duration%`, `Date.prototype.toTemporalInstant`, `Instant.prototype.add`/`subtract`/`round`, and `Duration.prototype.round` share that kernel. Instant arithmetic preserves resumable `ToTemporalDuration`, time-only/range checks, fresh results, string shorthand, and `roundingIncrement`/`roundingMode`/`smallestUnit` read order; Duration round preserves its option-read/coercion order, modes, increments, `relativeTo`, resource limits, and realm topology. Next: Instant difference/zoned operations, remaining Temporal types, binary data/typed arrays, and Atomics.
