...
- [~] `%eval%` compiles and executes a distinct whole-graph indirect-eval authority: non-strings return unchanged; completion values, Realm-global resolution, strict/sloppy declarations, retained eval lexicals, and catchable syntax/declaration errors follow `PerformEval`. The compiler now lowers identity-checked direct `eval`/spread callsites with QuickJS-compatible adjusted lexical-scope metadata, and the whole-graph verifier certifies their scope chain while raw bytecode remains fail closed; runtime caller-environment compilation/execution is still pending. Focused Test262 is 110/114, with the residual modes requiring direct eval or `with`; next complete strict and sloppy direct eval, then `with` object environments/`@@unscopables`.
