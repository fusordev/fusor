...
- [~] `Iterator`, `%Iterator.prototype%`, `Iterator.from`, opaque `%WrapForValidIteratorPrototype%` records, and `Iterator.prototype.toArray` implement subclass-only construction, `GetIteratorFlattenable`, one-time retained `next`, forwarded `return`, specified accessors, and resumable direct iteration without closing on step failures. Pinned Test262 `Iterator.from` is 38/38 and `toArray` is 36/36. Next add synchronous lazy helper records with callback/index and required `IteratorClose` semantics; keep async helpers separate until job-queue interaction has focused coverage.
...
- [~] Full custom-policy baseline at pinned Test262 `be13516fb6441b950ba8a3df97eb34062c186972` and policy fingerprint `84d947e4c80827a1`: 70,563/79,779 admitted cases pass (88.45%), 9,216 fail, and 23,133 are explicitly skipped. The largest admitted failure cohorts are `language` (4,394), Temporal (1,254), Iterator (1,094), `staging` (1,027), and Object (330); skips include low-priority ECMA-402 and unsupported host/module/proposal features, so this is a reproducible alignment baseline, not an unfiltered conformance claim.
