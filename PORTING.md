...
- [~] `%eval%` uses distinct whole-graph indirect/direct Script authorities. Identity-checked direct callsites retain caller/source strictness, adjusted lexical scope, ordinary-call fallback, and `GetThisEnvironment` context across nested eval and arrows for `new.target`, `super` properties, and no-instance-element `super()` calls. Sloppy function eval has a per-activation variable environment shared by nested eval and escaping closures; non-simple parameters use certified parameter/callee/body boundaries, including ordinary/arrow `arguments`, body-binding separation, dynamic shadowing of outer captures, and deletable eval-created vars/functions with missing-read/`typeof` behavior. Installation remains atomic with live-cell promotion and rollback; the global branch publishes configurable declarations with conflict checks. Sloppy `with` now lowers a fresh capturable object-environment cell and verified asymmetric lookup branch: reads, `typeof`, bare calls/spread/tags, and `delete` preserve inner-to-outer ordering, inherited/accessor/Proxy behavior, `@@unscopables`, strict `GetBindingValue`, call receivers, and abrupt completion. Writes and direct eval inside `with` remain fail closed. Pinned Test262 `language/eval-code` therefore remains 397/400 (99.25%); complete those reference forms, then post-deletion assignment re-resolution. Strict and sloppy direct eval are both required.
