...
- [~] `%eval%` now compiles and executes a distinct whole-graph indirect-eval authority: non-strings return unchanged; completion values, Realm-global resolution, strict/sloppy declarations, retained eval lexicals, and catchable syntax/declaration errors follow `PerformEval`. Focused Test262 is 110/114; the four residual modes require direct eval or `with`. Next add caller-context direct eval, `with` object environments/`@@unscopables`, remaining opcode families, and complete debug/source tables. Unverified bytecode remains fail closed.
