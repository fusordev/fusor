...
- [~] `Iterator`, `%Iterator.prototype%`, `Iterator.from`, `toArray`, helper transforms/consumers, `%Symbol.dispose%`, and `Iterator.concat` use retained records, resumable callbacks, GC tracing, and normative close/error ordering. `flatMap` closes active inner before outer; `concat` captures iterable methods eagerly, opens each iterator lazily, and closes only the active iterator. Pinned Test262: `from` 38/38, `toArray` 36/36, `map` 72/72, `filter` 74/74, admitted `take` 60/60 and `drop` 62/62, `flatMap` 88/88, `every` 66/66, `find` 64/64, `forEach` 54/54, `reduce` 60/60, `some` 66/66, admitted `%Symbol.dispose%` 12/12, and `concat` 64/64. Broader explicit-resource-management syntax remains skipped; six modes per limit helper are policy-excluded for the removed `MAX_SAFE_INTEGER` bound. Next add `zip`/`zipKeyed`; keep async helpers separate until job-queue interaction has focused coverage.
