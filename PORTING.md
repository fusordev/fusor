...
- [~] Temporal: `%Temporal.Instant%`, `%Temporal.Duration%`, `Date.prototype.toTemporalInstant`, and a branded `%Temporal.PlainDate%` core share `temporal_rs = 0.2.5`. PlainDate has ordered component/calendar coercion, new-target allocation, ISO/string/branded/property-bag `from`, calendar-derived accessors, ISO string/JSON/non-Intl locale output, and `equals`. Property bags preserve observable `calendar`/date-field conversion order and validate `overflow`; reuse that conversion for comparison, arithmetic, `with`, and remaining Temporal types is next.
