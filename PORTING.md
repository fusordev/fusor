...
- [~] Temporal: `%Temporal.Instant%`, `%Temporal.Duration%`, `Date.prototype.toTemporalInstant`, and branded `%Temporal.PlainDate%`/`%Temporal.PlainDateTime%` cores share `temporal_rs = 0.2.5`. PlainDate supports ordered conversion, string-only calendar validation, new-target allocation, ISO/string/branded/property-bag `from`/`compare`, calendar-derived accessors, optioned ISO string/JSON/non-Intl locale output, `equals`, date-duration `add`/`subtract`, partial-field `with`, `toPlainDateTime`, `withCalendar`, and `until`/`since`. PlainDateTime has its constructor, time defaults, core calendar/date/time accessors, optioned ISO string/JSON/non-Intl locale output, primitive rejection, ISO/string/branded/PlainDate/property-bag `from`/`compare`, `equals`, duration `add`/`subtract`, partial-field `with`, `round`, `toPlainDate`/`toPlainTime`, `withCalendar`, and `until`/`since`. PlainTime now has branded slots, constructor/new-target behavior, accessors, optioned ISO string/JSON/non-Intl locale output, primitive rejection, ISO/string/branded/PlainDateTime/property-bag `from`/`compare`/`equals`, duration `add`/`subtract`, partial-field `with`, `round`, and time-only rounded `until`/`since`; PlainDate bridges its PlainTime slots directly. Property bags preserve observable field conversion order; `from`/arithmetic defer overflow validation, `with` prepares fields before options, rounding reads/coerces all options resumably, differences convert the date/time operand before all four difference settings, and kernel Duration outputs normalize to ECMAScript Number fields. Calendar/ZonedDateTime and remaining Temporal types are next.
