...
- [~] Temporal (`temporal_rs = 0.2.5`): `%Temporal.Instant%`, `%Temporal.Duration%`, `Date.prototype.toTemporalInstant`, and branded PlainDate/PlainDateTime/PlainTime cores have constructors, accessors, optioned ISO/JSON/non-Intl output, ordered ISO/string/branded/property-bag conversion, arithmetic, field replacement, rounding/differences, calendar changes, and primitive rejection as applicable. PlainDate and PlainDateTime `toZonedDateTime` preserve observable option/property order and pass 92/92 and 58/58 focused cases. Duration is complete: `ToTemporalRelativeTo` accepts branded values, ISO strings, and property bags (time-zone bags route through the resumable ZonedDateTime field machinery; plain bags become PlainDate relativeTo with required-field errors), and constructor range checks fire at each argument's own conversion; focused Duration is 1080/1080. Every observable field/options read and coercion uses a rooted continuation; arithmetic/from defer overflow validation and Duration fields are ECMAScript Numbers.
...
- [~] Binary data: fixed/resizable and immutable `ArrayBuffer`, fixed/growable `SharedArrayBuffer`, and `DataView` (including Float16/BigInt and resizable-view witnesses) are complete. Immutable buffers have the proposal slots/intrinsics, transfer/slice operations, read-only typed-array reflection, early DataView/typed-array/Atomics write rejection, writable species validation, and `%TypedArray%.from`/`of`. Typed arrays otherwise retain the complete dense backing/GC/indexed-exotic, constructor/accessor/`@@species`, copying/search/iterator/callback/reduction, and resumable stable numeric sort surface. At pinned Test262 `be13516fb6441b950ba8a3df97eb34062c186972`, focused current-policy runs pass ArrayBuffer 408/408 and Atomics 444/444 admitted cases, all 42 immutable DataView/typed-array/Uint8 write modes, and `%TypedArray%.from` 114/114 plus `of` 54/54. Next: runtime-owned multi-agent waiter lists/wakeups and `Atomics.waitAsync`.
...
- [~] The last full custom-policy baseline at pinned Test262 `be13516fb6441b950ba8a3df97eb34062c186972` used historical policy fingerprint `208c24ffaa65423a`: 72,818/79,767 admitted cases pass (91.28%), 6,949 fail, and 23,145 are explicitly skipped. The current policy fingerprint is `b5695ef673ae34c4` after admitting Immutable ArrayBuffer; this focused tranche deliberately did not rerun the full suite. Seventeen historical case modes across nine paths reached the five-second host interrupt and remain accounted for as failures; skips include low-priority ECMA-402 and unsupported host/module/proposal features, so the baseline is reproducible alignment evidence, not an unfiltered conformance claim.
