...
- [~] Temporal: `%Temporal.Instant%`, `%Temporal.Duration%`, `Date.prototype.toTemporalInstant`, Instant arithmetic/difference/rounding, and Duration rounding share `temporal_rs = 0.2.5`. `Instant.prototype.until`/`since` convert `other` before their object-only options and preserve the normative `largestUnit` → `roundingIncrement` → `roundingMode` → `smallestUnit` Get/coercion sequence through resumable continuations; the kernel owns time-only validation, defaults, increments, and `since` rounding inversion. Next: Instant string/time-zone/zoned operations, remaining Temporal types, binary data/typed arrays, and Atomics.
