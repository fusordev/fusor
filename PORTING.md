...
- [~] Temporal: `%Temporal.Instant%`, `%Temporal.Duration%`, `Date.prototype.toTemporalInstant`, and a branded `%Temporal.PlainDate%` core share `temporal_rs = 0.2.5`. PlainDate has ordered component/calendar coercion, new-target allocation, ISO/string and branded `from`/`compare`, calendar-derived accessors, ISO string/JSON/non-Intl locale output, and `equals`; property bags, options, arithmetic, conversions, and the remaining Temporal types are next.
