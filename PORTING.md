...
- [~] ZonedDateTime has its initial constructor, string/branded `from`, branded slots/projections, `equals`/static `compare`, JSON/non-Intl output, primitive rejection, optioned `toString`, string-only `withTimeZone`, shared `ToTemporalTime` `withPlainTime`, and duration `add`/`subtract` with post-duration resumable `overflow` options. `toString` is 124/124; `withTimeZone` 32/32; `withPlainTime` 72/72; and `getTimeZoneTransition` 28/28. Arithmetic is 78/86 (`add`) and 76/84 (`subtract`): the only retained cases need still-absent property-bag `ZonedDateTime.from` or PlainDate/PlainDateTime `toZonedDateTime`, not arithmetic behavior. Focused ZonedDateTime is 1004/1796 (55.90%, pinned Test262 `be13516fb6441b950ba8a3df97eb34062c186972`); property-bag conversion, Calendar parsing, differences, rounding, and remaining Temporal types are next.
