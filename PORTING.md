...
- [~] Temporal: `%Temporal.Instant%`, `%Temporal.Duration%`, `Date.prototype.toTemporalInstant`, and branded `%Temporal.PlainDate%`/`%Temporal.PlainDateTime%` cores share `temporal_rs = 0.2.5`. PlainDate supports ordered conversion, string-only calendar validation, new-target allocation, ISO/string/branded/property-bag `from`/`compare`, calendar-derived accessors, ISO string/JSON/non-Intl locale output, `equals`, date-duration `add`/`subtract`, partial-field `with`, and `until`/`since`. PlainDateTime has its constructor, time defaults, core calendar/date/time accessors, ISO string/JSON/non-Intl locale output, primitive rejection, ISO/string/branded/PlainDate/property-bag `from`/`compare`, `equals`, and duration `add`/`subtract`. Property bags preserve observable field conversion order; `from`, arithmetic, and `with` validate `overflow` after input conversion, while differences convert the date operand before all four difference settings. PlainDateTime differences/`with`, Calendar/ZonedDateTime, and remaining Temporal types are next.
