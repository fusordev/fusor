...
- [~] The latest supplied full CI report at pinned Test262 `be13516fb6441b950ba8a3df97eb34062c186972` and current policy fingerprint `3f71d7cab8cd01d4` passes 77,495/79,745 admitted modes (97.18%), with 2,250 failures and 23,167 explicit skips across 53,575 files. Eight modes exhaust the 50M instruction budget; the remaining failures are semantic/compiler/host cohorts, not silently dropped inventory. The report format does not embed the engine commit, so this is the latest supplied CI baseline rather than a claim about every subsequent local commit.
