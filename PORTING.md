...
- [~] ZonedDateTime has its constructor (merged-spec identifier coercion: non-string time zones/calendars raise `TypeError`, ISO datetime time-zone strings raise `RangeError`); full string/branded/property-bag `from`; branded slots/projections; property-bag-capable `equals`/static `compare`; JSON/non-Intl output; optioned `toString`; string-only `withTimeZone`; shared `ToTemporalTime` `withPlainTime`; transitions; duration `add`/`subtract`; `withCalendar` (identifier/ISO strings and Temporal-object calendar slots); and partial-field `with` (spec-ordered reject-phase `calendar`/`timeZone` checks, field validation before options normalization, default `offset: "prefer"`, resumable disambiguation/offset/overflow reads); and `round` (ordered `roundingIncrement`/`roundingMode`/`smallestUnit` reads, time-zone-aware day rounding over start-of-day transitions, compatible re-interpretation for sub-day units). Property bags preserve all 13 ordered fields, Temporal calendar/time-zone fast paths, lexical `offset` validation, then resumable `disambiguation`/`offset`/`overflow` reads before kernel validation. `from` is 178/178; `toString` 124/124; `withTimeZone` 32/32; `withPlainTime` 72/72; transitions 28/28; `add` 86/86; `subtract` 84/84; `withCalendar` 28/28; `with` 68/68; and `round` 88/88. Focused ZonedDateTime is 1404/1796 (78.17%, pinned Test262 `be13516fb6441b950ba8a3df97eb34062c186972`); `until`/`since` differences and remaining Temporal types are next.
