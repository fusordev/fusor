...
- [~] `Iterator`, `%Iterator.prototype%`, `Iterator.from`, `toArray`, shared helper state for `map`/`filter`/`take`/`drop`/`flatMap`, and immediate consumers `every`/`find`/`forEach`/`some` implement retained direct records, resumable callbacks/indexes, GC tracing, and normative close/error ordering. `flatMap` persists an active inner record and closes inner before outer. Pinned Test262: `from` 38/38, `toArray` 36/36, `map` 72/72, `filter` 74/74, admitted `take` 60/60 and `drop` 62/62, `flatMap` 88/88, `every` 66/66, `find` 64/64, `forEach` 54/54, and `some` 66/66; six modes per limit helper are policy-excluded because they retain the removed `MAX_SAFE_INTEGER` bound. Next add `reduce`; keep async helpers separate until job-queue interaction has focused coverage.
