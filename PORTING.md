...
- [~] `%Intl%` and `Intl.getCanonicalLocales` implement resumable `CanonicalizeLocaleList`, UTS #35 aliases/extensions, ordered deduplication, and standard descriptors. Pinned `intl402/Intl/getCanonicalLocales` is 74/76; both residual modes are the single cross-feature `Locale-object.js`, pending `%Intl.Locale%`.
