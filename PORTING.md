...
- [~] The requested Annex B subset follows completed `eval`/`with`: B.2.2.1/.15/.16 (`substr` and the identity-preserving trim aliases), B.2.3.1-.3 (`getYear`, `setYear`, and the `toUTCString`-identical `toGMTString`), and B.2.4.1 (`RegExp.prototype.compile`) are complete. Pinned Test262 admits 142/142 `annexB/built-ins` modes; 336 unrequested modes remain policy-excluded and four `legacy-regexp` modes await B.1.2. B.1.2 and B.3 remain next. The normative object-initializer `__proto__` setter already emits verified `set_proto`; computed, shorthand, method, and accessor keys remain ordinary definitions.
