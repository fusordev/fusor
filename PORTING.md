...
- [~] Temporal: `%Temporal.Instant%`, `%Temporal.Duration%`, `Date.prototype.toTemporalInstant`, and a branded `%Temporal.PlainDate%` core share `temporal_rs = 0.2.5`. PlainDate has ordered component/calendar coercion, new-target allocation, ISO/string/branded/property-bag `from`/`compare`, calendar-derived accessors, ISO string/JSON/non-Intl locale output, `equals`, and date-duration `add`/`subtract`. Property bags preserve observable `calendar`/date-field conversion order, and `from`/arithmetic validate `overflow` after their input conversions; `with`, differences, broader conversions, and remaining Temporal types are next.
