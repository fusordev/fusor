...
- [~] `%eval%` compiles and executes a distinct whole-graph indirect-eval authority: non-strings return unchanged; completion values, Realm-global resolution, strict/sloppy declarations, retained eval lexicals, and catchable syntax/declaration errors follow `PerformEval`. Identity-checked direct callsites carry verified adjusted scope metadata and execute closed eval Scripts with caller/source strictness, caller `this`, eval-local lexical/strict-`var` declarations, ordinary-call fallback, and catchable syntax errors. Direct eval now has its own verified Script authority: caller-before-global name resolution emits shape-bound caller-cell sources plus Realm-global fallbacks, and descendants may only forward those slots. Runtime live-cell materialization, private capture, sloppy caller-variable declarations, contextual `new.target`/`super`, and spread `apply_eval` remain pending—strict and sloppy direct eval are both required before `with` object environments/`@@unscopables`. Raw bytecode remains fail closed; the pinned focused cohort remains 110/114 until the runtime environment tranche lands.
