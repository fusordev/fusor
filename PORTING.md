...
- [x] Binary data: fixed/resizable/immutable `ArrayBuffer`, fixed/growable `SharedArrayBuffer`, DataView (Float16/BigInt and resizable witnesses), and the complete typed-array surface. A thread-safe host handle aliases one Shared Data Block across runtimes; bytes, atomic RMW, growth, and FIFO waiter registration/notification share its critical section. Blocking waits use cross-runtime wakeups; `waitAsync` has rooted/limited waiter records, same-agent Promise-order preservation, and lazy Tokio deadline signals settled only by the runtime owner. At pinned Test262 `be13516fb6441b950ba8a3df97eb34062c186972`, focused policy runs pass ArrayBuffer 408/408, Atomics 512/512 (including `waitAsync` 68/68 locally admissible; 134 async-host cases remain runner-skipped), all 42 immutable-write modes, and `%TypedArray%.from` 114/114 plus `of` 54/54.
- [x] ECMA-402 is isolated behind `quickjs-intl` with direct ICU4X 2.2 data; JavaScript coercion and observable property order remain runtime-owned. At pinned Test262 `be13516fb6441b950ba8a3df97eb34062c186972`, the Intl-only audit improved from 3,256/6,692 to 6,682/6,692 standard-cap passes across 3,357 files, with 22 explicit host-API skips and zero runner errors. All ten residual modes are outside Intl semantics: six pass 6/6 with the focused 60-second/high-fuel cap, while four await the separate Annex B integration (`String.prototype.substr` twice and sloppy block functions twice).
...
- [x] `%Intl.RelativeTimeFormat%` covers subclass construction; normative locale/option and value/unit coercion order; Unicode numbering systems; long, short, and narrow ICU4X patterns; named and numeric forms; supported locales; ordered resolved slots; and number-part partitioning with singular unit labels. Its pinned cohort improved from the prior 0/158 baseline to 158/158 admitted modes across 80 files, with two host-API skips and zero runner errors.
- [x] `%Intl.ListFormat%` covers subclass construction; normative locale/option order and primitive-option rejection; conjunction, disjunction, and unit patterns in every width; supported locales; ordered resolved slots; generic iterable consumption; string-only element validation with exceptional `IteratorClose`; and exact element/literal parts backed by ICU4X. Its pinned cohort improved from the prior 2/160 baseline to 160/160 admitted modes across 81 files, with two host-API skips and zero runner errors.
- [x] `%Intl.DisplayNames%` covers prototype-first subclass construction; normative locale and option coercion; all six language/region/script/currency/calendar/date-time-field code grammars; supported locales; ordered resolved slots; brand-before-code coercion; ICU4X language, region, and script names; calendar inventory names; and exact code/none fallback. Its pinned cohort improved from the prior 8/112 baseline to 112/112 admitted modes across 57 files, with two host-API skips and zero runner errors.
- [x] `Intl.supportedValuesOf` implements resumable key coercion, fresh Realm arrays, aligned Collator inventories, mandated calendar/numbering-system/unit inventories, and canonical ICU4X time-zone enumeration. The DisplayNames acceptance modes now close its pinned cohort at 50/50 with zero runner errors.
- [x] `%Intl.Segmenter%` covers prototype-first subclass construction; data-backed locale availability; grapheme, word, and sentence boundaries in UTF-16 coordinates; lone-surrogate preservation; `%IntlSegmentsPrototype%.containing`; independent `%IntlSegmentIteratorPrototype%` iterators; exact segment-data property order and word metadata; and rooted hidden intrinsic/instance edges. Its pinned cohort improved from the prior 4/154 baseline to 154/154 admitted modes across 79 files, with four host-API skips and zero runner errors.
- [x] `%Intl.DurationFormat%` covers prototype-first subclass construction; the current 24-option observation order and numeric-style cascade; exact integral duration validation and fractional folding beyond binary64 precision; locale-derived digital separators; ordered parts; Temporal.Duration slot bypass, Temporal string parsing, and duration-like field coercion in current specification order. `Temporal.Duration.prototype.toLocaleString` constructs the same intrinsic semantics without observing duration getters. Its pinned cohorts are DurationFormat 220/220 and the Temporal locale hook 4/4, with zero skips or runner errors.
- [x] Locale-sensitive String and array hooks share the Intl engines instead of public constructors: `localeCompare` uses Collator resolution after receiver/argument coercion, locale case conversion validates the complete locale list and applies ICU4X special casing, Array forwards both arguments, and `%TypedArray%.prototype.toLocaleString` is the same function object as Array's method. Pinned cohorts are String 38/38, Array 4/4, and TypedArray 2/2 with zero skips or runner errors.
- [x] All eight Temporal locale hooks use intrinsic Intl semantics over branded slots: Duration delegates to DurationFormat; Instant and the six calendar/time types use DateTimeFormat's type-specific required/default fields, style conflict rules, calendar compatibility, and time-zone projection; ZonedDateTime supplies its canonical instance zone, rejects a `timeZone` option, and includes a zone name by default. Pinned `intl402/Temporal/*/prototype/toLocaleString` is 194/194 across 97 files with zero skips or runner errors.
