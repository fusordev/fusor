...
- [ ] Reuse one instance-element initializer body when an arrow performs the first `super()`; `fields-run-once-on-double-super.js` remains fail closed instead of skipping or duplicating fields. Then close the remaining non-`eval` gaps, `eval`-coupled class semantics, and diagnostic audit. Proposal decorators remain policy-excluded.
- [~] `%eval%` uses distinct whole-graph indirect/direct Script authorities. Indirect eval handles non-strings, completions, Realm globals, strict/sloppy declarations, retained lexicals, and catchable syntax/declaration errors. Identity-checked direct callsites carry verified adjusted-scope metadata, caller/source strictness and ordinary-call fallback. Direct eval resolves caller cells before Realm globals; compilation retains otherwise-unreferenced visible outer bindings, while runtime atomically promotes parameter/local slots, reuses inherited cells, rolls back failed installation, preserves TDZ/immutable policies, and keeps escaping eval closures live. Sloppy direct eval records its inherited variable environment: the global branch atomically publishes configurable vars/functions with conflict/error checks; the function branch reuses existing body-var/parameter cells and retains newly declared vars/functions in a shared per-activation cell environment across nested eval and escaping closures. Pinned Test262 `language/eval-code` is 246/400; 139/154 failures are the still-separate parameter-initializer/`arguments` environment cohort. Finish that environment, private/contextual `new.target`/`super`, spread `apply_eval`, and remaining initialization cases before `with` object environments/`@@unscopables`. Strict and sloppy direct eval are both required; no non-strict exclusion is permitted.
