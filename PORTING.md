...
- [~] `Iterator`, `%Iterator.prototype%`, `Iterator.from`, and opaque `%WrapForValidIteratorPrototype%` records now implement subclass-only construction, `GetIteratorFlattenable`, retained `next`, forwarded `return`, and the specified constructor/`@@toStringTag` accessors. Pinned Test262 `Iterator.from` is 36/38; both residual cases only call the pending `Iterator.prototype.toArray`. Next add the synchronous lazy helper records, callback/index semantics, `toArray`, and mandatory `IteratorClose`; keep async helpers separate until job-queue interaction has focused coverage.
