...
- [~] The requested Annex B subset follows completed `eval`/`with`: B.1.2; B.2.2.1/.15/.16, B.2.3.1-.3, and B.2.4.1; and B.3.1-.5 are complete. Sloppy block, switch-case, and `if` functions use distinct lexical closures plus only the spec-permitted variable-environment replacement binding; declaration collisions, parameters, catch scopes, `arguments`, strict mode, and forbidden placements retain their normative boundaries. Catch `var` declarations install in the variable environment while matching initializers and references resolve through the catch cell. At pinned Test262 `be13516fb6441b950ba8a3df97eb34062c186972`, focused checked-policy runs pass function-code 159/159, global-code 136/136 admitted modes (17 host-API skips), eval-code 468/468 (two host-API skips), labelled 1/1, catch 10/10, and `for-in` 12/12; general catch destructuring passes 186/186. The merged `annexB` policy has not been rerun, so no combined-rate claim is made. B.3.6-.9 remain. Legacy RegExp literal/eval paths retain exact UTF-16, and `RegExp.prototype.compile` rejects subclass and cross-realm receivers. The normative object-initializer `__proto__` setter emits verified `set_proto`; computed, shorthand, method, and accessor keys remain ordinary definitions.
