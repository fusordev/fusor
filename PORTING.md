...
- [~] `IndirectEval` now produces a distinct whole-graph bytecode authority: lexical declarations stay eval-local, sloppy `var`/function declarations target the Realm global environment, and strict eval keeps every declaration local. Next install and execute that authority through `%eval%`, then add caller-context direct eval, `with` object environments/`@@unscopables`, remaining opcode families, and complete debug/source tables. Unverified bytecode remains fail closed.
