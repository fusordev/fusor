...
- [~] ZonedDateTime has its initial constructor, string/branded `from`, branded slots/projections, `equals`/static `compare`, JSON/non-Intl output, primitive rejection, optioned `toString`, and string-only `withTimeZone`. `toString` observes/coerces all six options before validation (124/124); `withTimeZone` is 32/32; `getTimeZoneTransition` accepts the string/options forms, observes only `direction`, returns compiled-IANA transitions or `null`, and is 28/28. Focused ZonedDateTime is 792/1796 (44.09%, pinned Test262 `be13516fb6441b950ba8a3df97eb34062c186972`); property-bag conversion, Calendar parsing, arithmetic, and remaining Temporal types are next.
