...
- [~] `%eval%` uses distinct whole-graph indirect/direct Script authorities. Indirect eval handles non-strings, completions, Realm globals, strict/sloppy declarations, retained lexicals, and catchable syntax/declaration errors. Identity-checked direct callsites carry verified adjusted-scope metadata, caller/source strictness and ordinary-call fallback. Direct eval resolves caller cells before Realm globals; compilation retains otherwise-unreferenced visible outer bindings, while runtime atomically promotes parameter/local slots, reuses inherited cells, rolls back failed installation, preserves TDZ/immutable policies, and keeps escaping eval closures live. Sloppy direct eval now records its inherited variable-environment kind; the global branch publishes configurable vars/functions, preflights declarations atomically, rejects global/intervening lexical conflicts, maps rejected properties to catchable errors, and survives nested eval. Pinned Test262 `language/eval-code` is 239/400; 142/161 failures remain the deliberately fail-closed function/parameter variable environments. Finish those shared declarative environments, `arguments`/private/contextual `new.target`/`super`, spread `apply_eval`, and the remaining initialization cases before `with` object environments/`@@unscopables`. Strict and sloppy direct eval are both required; no non-strict exclusion is permitted.
