...
- [x] `cargo xtask test262` and the manual-dispatch GitHub Actions workflow clone current upstream Test262, apply only the repository's custom non-Annex-B filter policy (including Temporal), print the revision and full-suite pass rate, and upload the deterministic JSON report.
