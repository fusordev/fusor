...
- [~] ZonedDateTime has its initial constructor, string/branded `from`, branded slots/projections, `equals`/static `compare`, JSON/non-Intl output, primitive rejection, optioned `toString`, string-only `withTimeZone`, and shared `ToTemporalTime` `withPlainTime`. `toString` observes/coerces all six options before validation (124/124); `withTimeZone` is 32/32; `withPlainTime` is 72/72; `getTimeZoneTransition` accepts the string/options forms, observes only `direction`, and returns compiled-IANA transitions or `null` (28/28). Focused ZonedDateTime is 862/1796 (47.99%, pinned Test262 `be13516fb6441b950ba8a3df97eb34062c186972`); property-bag conversion, Calendar parsing, arithmetic, and remaining Temporal types are next.
