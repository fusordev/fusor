...
- [~] `%eval%` uses distinct whole-graph indirect/direct Script authorities. Identity-checked direct callsites retain caller/source strictness, adjusted lexical scope, and ordinary-call fallback. Sloppy function eval now has a per-activation variable environment shared by nested eval and escaping closures; non-simple parameters use certified parameter/callee/body environment boundaries, including ordinary/arrow `arguments`, body-binding separation, dynamic shadowing of outer captures, and deletable eval-created vars/functions with missing-read/`typeof` behavior. Installation remains atomic with live-cell promotion and rollback; the global branch publishes configurable declarations with conflict checks. Pinned Test262 `language/eval-code` is 393/400 (98.25%): only contextual direct-eval `new.target` (2 modes), `super` property (2), and the three `with` cases remain. Complete contextual syntax and post-deletion assignment re-resolution first, then `with` object environments/`@@unscopables`; strict and sloppy direct eval are both required with no non-strict exclusion.
