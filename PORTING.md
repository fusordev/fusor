...
- [~] Private instance and static **data fields**, plus ordinary private instance methods, have fresh opaque names and VM-only own slots. A private method closure is created once per class evaluation, receives `#name` and the class prototype as its home object, and is installed on each instance; direct calls, `super`, bad-receiver `TypeError`, and `#name in object` are covered. Slots are non-enumerable/non-configurable, invisible to string reflection, do not walk prototypes, and do not invoke Proxy traps.
- [ ] Close classes: private accessors and static private methods; compound/logical/update private writes; decorators; and the remaining private-function naming/diagnostic audit. Arrow-contained `super()` is supported. Parsing is not execution support.
