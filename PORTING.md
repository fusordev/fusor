...
- [~] The requested Annex B subset follows completed `eval`/`with`: B.1.2 legacy RegExp grammar; B.2.2.1/.15/.16, B.2.3.1-.3, and B.2.4.1; B.3.4 source and direct-eval catch-parameter `var` redeclarations; and B.3.5 `for-in` initializers are complete. B.3.4 splits declaration instantiation from identifier resolution: declarations install in the variable environment, while initializers, reads, loops, destructuring, closures, and intervening `with` objects resolve through the matching catch cell; other lexical collisions still reject eval. At pinned Test262 `be13516fb6441b950ba8a3df97eb34062c186972`, B.3.4/.5 pass 23/23 modes. The combined checked-in `annexB` policy passes 203/207 at the generic 10M fuel; all four BMP RegExp stress modes pass at 50M, while 1,168 baseline-excluded and two unsupported-host modes remain skipped. B.3.1-.3 and the B.3.6-.9 host/runtime boundaries remain. Legacy RegExp literal/eval paths retain exact UTF-16, and `RegExp.prototype.compile` rejects subclass and cross-realm receivers. The normative object-initializer `__proto__` setter emits verified `set_proto`; computed, shorthand, method, and accessor keys remain ordinary definitions.
