...
- [~] `%eval%` compiles and executes a distinct whole-graph indirect-eval authority: non-strings return unchanged; completion values, Realm-global resolution, strict/sloppy declarations, retained eval lexicals, and catchable syntax/declaration errors follow `PerformEval`. Identity-checked direct `eval` callsites now carry verified adjusted lexical-scope metadata and execute closed eval Scripts with caller/source strictness, caller `this`, eval-local lexical/strict-`var` declarations, ordinary-call fallback, and catchable syntax errors; raw bytecode remains fail closed. Caller/global/private binding capture, sloppy caller-variable declarations, contextual `new.target`/`super`, and spread `apply_eval` remain pending—strict and sloppy direct eval are both required before `with` object environments/`@@unscopables`. The pinned focused cohort remains 110/114 until that environment tranche lands.
