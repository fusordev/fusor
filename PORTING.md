...
- [ ] Close the remaining non-`eval` class execution gaps, including arrow-contained repeated `super()` and its shared derived-`this` environment, then the `eval`-coupled class semantics and diagnostic audit. Proposal decorators remain policy-excluded. Parsing alone is not execution support.
