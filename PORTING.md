...
- [~] Classes: named base/derived declarations and expressions; inheritance (including `extends null`); explicit/synthesized constructors; `super(...)` including spread; public static/instance fields and methods/accessors; computed field keys; static initialization blocks; and direct/computed `super` reads, calls, simple/compound/logical writes, and updates. Field/static-block lexical `this`, `super`, `new.target`, arrows, source order, and derived receiver timing have focused coverage. Private methods, accessors, and static private elements compile and execute; SetHomeObject/CheckBrand/AddBrand are implemented in the VM.
- [~] Private **instance data fields** (`#x = initializer`) have fresh opaque names and VM-only own slots. Direct read/simple write and `#x in object` are supported; slots are non-enumerable/non-configurable, invisible to string reflection, do not walk prototypes, and do not invoke Proxy traps. Missing brands and primitive receivers throw `TypeError`. Private methods/accessors/static-private execute with brand checks.
