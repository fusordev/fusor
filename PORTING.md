...
- [~] Temporal: `%Temporal.Instant%`, `%Temporal.Duration%`, `Date.prototype.toTemporalInstant`, and a branded `%Temporal.PlainDate%` core share `temporal_rs = 0.2.5`. PlainDate has ordered component/calendar coercion, new-target allocation, ISO/string/branded/property-bag `from`/`compare`, calendar-derived accessors, ISO string/JSON/non-Intl locale output, `equals`, date-duration `add`/`subtract`, partial-field `with`, and `until`/`since`. Property bags preserve observable field conversion order; `from`, arithmetic, and `with` validate `overflow` after input conversion, while differences convert the date operand before all four difference settings. Broader conversions and remaining Temporal types are next.
