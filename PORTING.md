...
- [ ] Add the `Iterator` intrinsic and synchronous Iterator Helpers: `Iterator.from`, `%Iterator.prototype%`, lazy helper iterator records, callback/index semantics, and mandatory `IteratorClose` behavior. Keep async iterator helpers separate until the async-iteration/job-queue interaction has focused coverage.
