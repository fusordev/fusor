...
- [~] Temporal: `%Temporal.Instant%`, `%Temporal.Duration%`, `Date.prototype.toTemporalInstant`, `Instant.prototype.add`/`subtract`, and `Duration.prototype.round` share that kernel. Instant arithmetic preserves resumable `ToTemporalDuration`, time-only/range checks, and fresh results; Duration round preserves option-read/coercion order, string shorthand, modes, increments, `relativeTo`, resource limits, and realm topology. Next: Instant round/difference/zoned operations, remaining Temporal types, binary data/typed arrays, and Atomics.
