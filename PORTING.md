...
- [~] `Iterator`, `%Iterator.prototype%`, `Iterator.from`, `Iterator.prototype.toArray`, and the `%IteratorHelperPrototype%`/`map` path implement retained direct records, lazy resumable mapping, indexed callbacks, reentrancy rejection, GC tracing, and the required close/error ordering. Pinned Test262 is 38/38 for `from`, 36/36 for `toArray`, and 72/72 for `map`. Next extend the shared helper state through `filter`/`take`/`drop`/`flatMap` and consuming helpers; keep async helpers separate until job-queue interaction has focused coverage.
