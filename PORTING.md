...
- [~] Temporal: `%Temporal.Instant%`, `%Temporal.Duration%`, `Date.prototype.toTemporalInstant`, Instant arithmetic/difference/rounding/string formatting, and Duration rounding share `temporal_rs = 0.2.5`. Instant difference converts `other` before object-only options and preserves the normative `largestUnit` → `roundingIncrement` → `roundingMode` → `smallestUnit` sequence; `Instant.prototype.toString` now preserves its `fractionalSecondDigits` → `roundingMode` → `smallestUnit` → `timeZone` sequence and formats IANA/fixed-offset zones through the pinned compiled data. Next: Zoned operations, remaining Temporal types, binary data/typed arrays, and Atomics.
