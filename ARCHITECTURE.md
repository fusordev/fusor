...
The runtime does not depend on Oxc or Tokio. It depends on the project-owned
diagnostics data layer so verified exception provenance can become stable host
diagnostics without making Miette output semantic truth. The compiler consumes
an Oxc AST and produces an owned verified bytecode unit. The Tokio adapter
delivers owned host completions to the runtime owner; it never accesses the
JavaScript heap.
