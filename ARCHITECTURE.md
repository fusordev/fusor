...
produces an owned verified bytecode unit. Its narrow Tokio dependency provides
finite `Atomics.waitAsync` deadline signals; only the runtime owner touches the
JavaScript heap, settles promises, and orders jobs. The planned general Tokio
adapter follows the same owned-completion boundary.
